...
impl PLGraph {
    fn transition_matrix(&self) -> [[f64; N_TOK]; N_TOK] {
        ...
        for i in 0..N_TOK {
            ...
            if row_sum > 0.0 {
                for j in 0..N_TOK { p[i][j] = self.weights[i][j] / row_sum; }
            } else { p[i][i] = 1.0; }
        }
        p
    }
        ...
        for start in 0..N_TOK {
            ...
            for _ in 0..tau {
                ...
                for i in 0..N_TOK {
                    ...
                    for j in 0..N_TOK { next[j] += pi[i] * p[i][j]; }
                }
                pi = next;
            }
            let mut h = 0.0f64;
...
fn jacobi_eigen_5(mat: &[[f64; N_DIM]; N_DIM]) -> [f64; N_DIM] {
    ...
    for _ in 0..100 {
        ...
        for ii in 0..N_DIM { for j in (ii+1)..N_DIM {
            if a[ii][j].abs() > max_off { max_off = a[ii][j].abs(); p = ii; q = j; }
        }}
    ...
    for (new_i, &(_, val)) in evals.iter().enumerate() { sorted[new_i] = val; }
    sorted
}
...
fn evaluate(g: &PLGraph) -> Metrics {
    ...
    let mean_s3 = if active_s3.is_empty() { 0.0 } else {
        active_s3.iter().sum::<f64>() / active_s3.len() as f64
    };

    let mut jsd_total = 0.0;
    let mut jsd_count = 0;
    for ii in 0..N_TOK { for j in (ii+1)..N_TOK {
        let mut jsd = 0.0;
        for k in 0..N_TOK {
            let m = (p[ii][k] + p[j][k]) / 2.0;
            if m > 1e-30 {
                if p[ii][k] > 1e-30 { jsd += p[ii][k] * (p[ii][k] / m).log2(); }
    ...
    let jsd = if jsd_count > 0 { jsd_total / jsd_count as f64 } else { 0.0 };
    ...
    for tok in 0..N_TOK {
        ...
        let in_deg = if total_w > 0.0 { col_sum / total_w } else { 0.0 };
        let mut out_h = 0.0f64;
        for j in 0..N_TOK { if p[tok][j] > 1e-30 { out_h -= p[tok][j] * p[tok][j].log2(); } }
        active.push([s1[tok], s3[tok], s5[tok], in_deg, out_h]);
    }

    let n = active.len();
    if n < 3 { return Metrics { mean_s3, jsd, phi: 0.0, d_eff: 0, evals: [0.0; N_DIM] }; }

    let mut means = [0.0f64; N_DIM];
    let mut stds = [0.0f64; N_DIM];
    for d in 0..N_DIM { means[d] = active.iter().map(|w| w[d]).sum::<f64>() / n as f64; }
    for d in 0..N_DIM {
        let var = active.iter().map(|w| (w[d] - means[d]).powi(2)).sum::<f64>() / n as f64;
        stds[d] = var.sqrt();
    }
    ...
    for a in 0..N_DIM { for b in 0..N_DIM {
        if stds[a] < 1e-12 || stds[b] < 1e-12 {
            ...
            let mut s = 0.0;
            for w in &active { s += (w[a] - means[a]) * (w[b] - means[b]); }
    ...
    if total < 1e-15 { return Metrics { mean_s3, jsd, phi: 0.0, d_eff: 0, evals }; }
    let mut cum = 0.0;
    let mut d_eff = N_DIM;
    for (ii, &e) in evals.iter().enumerate() {
        cum += e;
        if cum / total >= 0.80 { d_eff = (ii + 1).min(N_DIM); break; }
    }
    let phi = if d_eff >= 2 { evals.iter().take(d_eff).sum::<f64>() / total } else { 0.0 };
    Metrics { mean_s3, jsd, phi, d_eff, evals }
}

fn token_entropy(g: &PLGraph, tok: usize) -> f64 {
    let p = g.transition_matrix();
    (0..N_TOK).map(|j| {
        if p[tok][j] > 1e-30 { -p[tok][j] * p[tok][j].log2() } else { 0.0 }
    }).sum()
}

fn token_role(h: f64) -> &'static str {
    if h < 0.8 { "tunnel" }
    else if h < 1.5 { "guided" }
    else if h < 2.5 { "junction" }
    else { "hub" }
}
...
fn make_lang(name: &'static str) -> PLGraph {
    ...
    match name {
        "Rust" => {
            w[i("START")][i("KW")] = 25.0; w[i("START")][i("IDENT")] = 20.0;
            w[i("START")][i("TYPE")] = 10.0;
            w[i("KW")][i("IDENT")] = 30.0; w[i("KW")][i("TYPE")] = 15.0;
            w[i("KW")][i("KW")] = 8.0; w[i("KW")][i("BRACE")] = 5.0;
            w[i("IDENT")][i("COLON")] = 15.0; w[i("IDENT")][i("OP")] = 20.0;
            w[i("IDENT")][i("PAREN")] = 15.0; w[i("IDENT")][i("DOT")] = 10.0;
            w[i("IDENT")][i("SEMI")] = 8.0; w[i("IDENT")][i("COMMA")] = 8.0;
            w[i("IDENT")][i("ARROW")] = 5.0; w[i("IDENT")][i("BRACE")] = 5.0;
            w[i("OP")][i("IDENT")] = 25.0; w[i("OP")][i("LIT")] = 15.0;
            w[i("OP")][i("PAREN")] = 5.0; w[i("OP")][i("TYPE")] = 5.0;
            w[i("TYPE")][i("BRACE")] = 10.0; w[i("TYPE")][i("COMMA")] = 8.0;
            w[i("TYPE")][i("PAREN")] = 8.0; w[i("TYPE")][i("OP")] = 5.0;
            w[i("TYPE")][i("ARROW")] = 5.0; w[i("TYPE")][i("SEMI")] = 3.0;
            w[i("PAREN")][i("IDENT")] = 15.0; w[i("PAREN")][i("TYPE")] = 10.0;
            w[i("PAREN")][i("LIT")] = 8.0; w[i("PAREN")][i("PAREN")] = 5.0;
            w[i("PAREN")][i("ARROW")] = 8.0;
            w[i("BRACE")][i("KW")] = 15.0; w[i("BRACE")][i("IDENT")] = 15.0;
            w[i("BRACE")][i("BRACE")] = 5.0; w[i("BRACE")][i("END")] = 5.0;
            w[i("SEMI")][i("KW")] = 15.0; w[i("SEMI")][i("IDENT")] = 15.0;
            w[i("SEMI")][i("BRACE")] = 5.0; w[i("SEMI")][i("END")] = 3.0;
            w[i("ARROW")][i("TYPE")] = 25.0; w[i("ARROW")][i("IDENT")] = 5.0;
            w[i("COLON")][i("TYPE")] = 25.0; w[i("COLON")][i("COLON")] = 8.0;
            w[i("COLON")][i("IDENT")] = 5.0;
            w[i("DOT")][i("IDENT")] = 30.0;
            w[i("COMMA")][i("IDENT")] = 15.0; w[i("COMMA")][i("TYPE")] = 10.0;
            w[i("COMMA")][i("LIT")] = 5.0;
            w[i("LIT")][i("OP")] = 10.0; w[i("LIT")][i("COMMA")] = 8.0;
            w[i("LIT")][i("SEMI")] = 8.0; w[i("LIT")][i("PAREN")] = 3.0;
        }
        "Prolog" => {
            w[i("START")][i("IDENT")] = 35.0; w[i("START")][i("KW")] = 5.0;
            w[i("START")][i("COLON")] = 3.0;
            w[i("IDENT")][i("PAREN")] = 25.0; w[i("IDENT")][i("COMMA")] = 15.0;
            w[i("IDENT")][i("DOT")] = 12.0; w[i("IDENT")][i("OP")] = 10.0;
            w[i("IDENT")][i("ARROW")] = 8.0; w[i("IDENT")][i("SEMI")] = 3.0;
            w[i("IDENT")][i("DELIM")] = 3.0; w[i("IDENT")][i("END")] = 2.0;
            w[i("PAREN")][i("IDENT")] = 22.0; w[i("PAREN")][i("LIT")] = 10.0;
            w[i("PAREN")][i("DELIM")] = 8.0; w[i("PAREN")][i("PAREN")] = 5.0;
            w[i("PAREN")][i("ARROW")] = 10.0; w[i("PAREN")][i("COMMA")] = 12.0;
            w[i("PAREN")][i("DOT")] = 8.0;
            w[i("ARROW")][i("IDENT")] = 30.0; w[i("ARROW")][i("PAREN")] = 5.0;
            w[i("ARROW")][i("OP")] = 3.0;
            w[i("COMMA")][i("IDENT")] = 28.0; w[i("COMMA")][i("OP")] = 5.0;
            w[i("COMMA")][i("PAREN")] = 3.0; w[i("COMMA")][i("LIT")] = 5.0;
            w[i("SEMI")][i("IDENT")] = 25.0; w[i("SEMI")][i("PAREN")] = 5.0;
            w[i("DOT")][i("END")] = 30.0;
            w[i("OP")][i("IDENT")] = 22.0; w[i("OP")][i("LIT")] = 15.0;
            w[i("OP")][i("PAREN")] = 5.0; w[i("OP")][i("DELIM")] = 5.0;
            w[i("DELIM")][i("IDENT")] = 18.0; w[i("DELIM")][i("LIT")] = 10.0;
            w[i("DELIM")][i("DELIM")] = 8.0; w[i("DELIM")][i("PAREN")] = 3.0;
            w[i("LIT")][i("COMMA")] = 12.0; w[i("LIT")][i("OP")] = 8.0;
            w[i("LIT")][i("DELIM")] = 5.0; w[i("LIT")][i("PAREN")] = 5.0;
            w[i("LIT")][i("DOT")] = 3.0; w[i("LIT")][i("END")] = 3.0;
            w[i("KW")][i("IDENT")] = 15.0; w[i("KW")][i("PAREN")] = 10.0;
            w[i("KW")][i("COLON")] = 3.0;
            w[i("COLON")][i("IDENT")] = 20.0; w[i("COLON")][i("KW")] = 5.0;
        }
        "Haskell" => {
            w[i("START")][i("IDENT")] = 25.0; w[i("START")][i("KW")] = 20.0;
            w[i("START")][i("TYPE")] = 5.0;
            w[i("KW")][i("IDENT")] = 25.0; w[i("KW")][i("KW")] = 10.0;
            w[i("KW")][i("TYPE")] = 8.0; w[i("KW")][i("PAREN")] = 5.0;
            w[i("IDENT")][i("IDENT")] = 15.0; w[i("IDENT")][i("OP")] = 15.0;
            w[i("IDENT")][i("PAREN")] = 10.0; w[i("IDENT")][i("COLON")] = 10.0;
            w[i("IDENT")][i("ARROW")] = 8.0; w[i("IDENT")][i("COMMA")] = 5.0;
            w[i("IDENT")][i("END")] = 5.0;
            w[i("OP")][i("IDENT")] = 20.0; w[i("OP")][i("LIT")] = 12.0;
            w[i("OP")][i("PAREN")] = 8.0; w[i("OP")][i("TYPE")] = 5.0;
            w[i("TYPE")][i("ARROW")] = 25.0; w[i("TYPE")][i("IDENT")] = 8.0;
            w[i("TYPE")][i("PAREN")] = 8.0; w[i("TYPE")][i("COMMA")] = 5.0;
            w[i("TYPE")][i("TYPE")] = 5.0;
            w[i("ARROW")][i("TYPE")] = 20.0; w[i("ARROW")][i("IDENT")] = 15.0;
            w[i("ARROW")][i("KW")] = 5.0;
            w[i("PAREN")][i("IDENT")] = 15.0; w[i("PAREN")][i("LIT")] = 8.0;
            w[i("PAREN")][i("OP")] = 8.0; w[i("PAREN")][i("PAREN")] = 5.0;
            w[i("DOT")][i("IDENT")] = 20.0;
            w[i("COLON")][i("TYPE")] = 25.0; w[i("COLON")][i("IDENT")] = 3.0;
            w[i("COMMA")][i("IDENT")] = 15.0; w[i("COMMA")][i("TYPE")] = 5.0;
            w[i("LIT")][i("OP")] = 10.0; w[i("LIT")][i("COMMA")] = 5.0;
            w[i("LIT")][i("END")] = 5.0; w[i("LIT")][i("COLON")] = 3.0;
        }
        "Catala" => {
            w[i("START")][i("KW")] = 42.0; w[i("START")][i("IDENT")] = 5.0;
            w[i("START")][i("OP")] = 3.0;
            w[i("KW")][i("KW")] = 30.0; w[i("KW")][i("IDENT")] = 20.0;
            w[i("KW")][i("TYPE")] = 8.0; w[i("KW")][i("PAREN")] = 3.0;
            w[i("KW")][i("COLON")] = 5.0; w[i("KW")][i("LIT")] = 5.0;
            w[i("IDENT")][i("DOT")] = 20.0; w[i("IDENT")][i("OP")] = 18.0;
            w[i("IDENT")][i("KW")] = 12.0; w[i("IDENT")][i("COMMA")] = 8.0;
            w[i("IDENT")][i("COLON")] = 8.0; w[i("IDENT")][i("PAREN")] = 3.0;
            w[i("IDENT")][i("END")] = 5.0;
            w[i("DOT")][i("IDENT")] = 38.0;
            w[i("COLON")][i("TYPE")] = 20.0; w[i("COLON")][i("KW")] = 15.0;
            w[i("COLON")][i("IDENT")] = 5.0;
            w[i("TYPE")][i("KW")] = 15.0; w[i("TYPE")][i("OP")] = 8.0;
            w[i("TYPE")][i("COMMA")] = 5.0; w[i("TYPE")][i("END")] = 3.0;
            w[i("OP")][i("IDENT")] = 18.0; w[i("OP")][i("LIT")] = 15.0;
            w[i("OP")][i("KW")] = 8.0; w[i("OP")][i("PAREN")] = 5.0;
            w[i("LIT")][i("KW")] = 12.0; w[i("LIT")][i("OP")] = 10.0;
            w[i("LIT")][i("COMMA")] = 5.0; w[i("LIT")][i("END")] = 5.0;
            w[i("PAREN")][i("IDENT")] = 12.0; w[i("PAREN")][i("LIT")] = 8.0;
            w[i("PAREN")][i("KW")] = 5.0; w[i("PAREN")][i("PAREN")] = 3.0;
            w[i("COMMA")][i("IDENT")] = 15.0; w[i("COMMA")][i("LIT")] = 8.0;
            w[i("COMMA")][i("KW")] = 5.0;
            w[i("SEMI")][i("KW")] = 20.0; w[i("SEMI")][i("IDENT")] = 5.0;
            w[i("DELIM")][i("IDENT")] = 8.0; w[i("DELIM")][i("LIT")] = 5.0;
        }
...
fn make_tau_v3() -> PLGraph {
    let mut w = [[0.0f64; N_TOK]; N_TOK];
    let i = PLGraph::idx;
    w[i("START")][i("KW")] = 35.0; w[i("START")][i("IDENT")] = 10.0;
    w[i("START")][i("DELIM")] = 5.0;
    w[i("KW")][i("IDENT")] = 22.0; w[i("KW")][i("BRACE")] = 16.0;
    w[i("KW")][i("PAREN")] = 10.0; w[i("KW")][i("KW")] = 5.0;
    w[i("KW")][i("TYPE")] = 5.0; w[i("KW")][i("COLON")] = 3.0;
    w[i("IDENT")][i("PAREN")] = 15.0; w[i("IDENT")][i("COLON")] = 13.0;
    w[i("IDENT")][i("ARROW")] = 13.0; w[i("IDENT")][i("DOT")] = 10.0;
    w[i("IDENT")][i("COMMA")] = 10.0; w[i("IDENT")][i("OP")] = 10.0;
    w[i("IDENT")][i("BRACE")] = 5.0; w[i("IDENT")][i("SEMI")] = 4.0;
    w[i("IDENT")][i("END")] = 3.0;
    w[i("COLON")][i("TYPE")] = 42.0; w[i("COLON")][i("IDENT")] = 5.0;
    w[i("SEMI")][i("TYPE")] = 25.0; w[i("SEMI")][i("KW")] = 8.0;
    w[i("SEMI")][i("IDENT")] = 5.0;
    w[i("TYPE")][i("ARROW")] = 25.0; w[i("TYPE")][i("COMMA")] = 12.0;
    w[i("TYPE")][i("PAREN")] = 10.0; w[i("TYPE")][i("BRACE")] = 6.0;
    w[i("TYPE")][i("OP")] = 5.0; w[i("TYPE")][i("SEMI")] = 3.0;
    w[i("TYPE")][i("END")] = 2.0;
    w[i("ARROW")][i("IDENT")] = 15.0; w[i("ARROW")][i("TYPE")] = 12.0;
    w[i("ARROW")][i("BRACE")] = 12.0; w[i("ARROW")][i("KW")] = 8.0;
    w[i("ARROW")][i("LIT")] = 8.0; w[i("ARROW")][i("DELIM")] = 5.0;
    w[i("ARROW")][i("PAREN")] = 5.0;
    w[i("DOT")][i("IDENT")] = 25.0; w[i("DOT")][i("KW")] = 3.0;
    w[i("DOT")][i("END")] = 8.0;
    w[i("PAREN")][i("IDENT")] = 15.0; w[i("PAREN")][i("LIT")] = 10.0;
    w[i("PAREN")][i("KW")] = 5.0; w[i("PAREN")][i("PAREN")] = 5.0;
    w[i("PAREN")][i("ARROW")] = 10.0; w[i("PAREN")][i("COMMA")] = 8.0;
    w[i("PAREN")][i("COLON")] = 8.0; w[i("PAREN")][i("BRACE")] = 5.0;
    w[i("PAREN")][i("DOT")] = 5.0;
    w[i("BRACE")][i("KW")] = 20.0; w[i("BRACE")][i("IDENT")] = 15.0;
    w[i("BRACE")][i("BRACE")] = 5.0; w[i("BRACE")][i("ARROW")] = 5.0;
    w[i("BRACE")][i("DOT")] = 5.0; w[i("BRACE")][i("LIT")] = 3.0;
    w[i("BRACE")][i("END")] = 5.0;
    w[i("COMMA")][i("IDENT")] = 20.0; w[i("COMMA")][i("LIT")] = 10.0;
    w[i("COMMA")][i("KW")] = 5.0; w[i("COMMA")][i("TYPE")] = 5.0;
    w[i("COMMA")][i("PAREN")] = 3.0; w[i("COMMA")][i("OP")] = 5.0;
    w[i("OP")][i("IDENT")] = 18.0; w[i("OP")][i("LIT")] = 15.0;
    w[i("OP")][i("PAREN")] = 8.0; w[i("OP")][i("KW")] = 3.0;
    w[i("OP")][i("BRACE")] = 3.0; w[i("OP")][i("DELIM")] = 3.0;
    w[i("LIT")][i("OP")] = 8.0; w[i("LIT")][i("COMMA")] = 10.0;
    w[i("LIT")][i("PAREN")] = 5.0; w[i("LIT")][i("ARROW")] = 6.0;
    w[i("LIT")][i("SEMI")] = 3.0; w[i("LIT")][i("COLON")] = 3.0;
    w[i("LIT")][i("END")] = 5.0;
    w[i("DELIM")][i("KW")] = 15.0; w[i("DELIM")][i("LIT")] = 15.0;
    w[i("DELIM")][i("IDENT")] = 12.0; w[i("DELIM")][i("DELIM")] = 3.0;
    PLGraph { name: "Futuruna-v3", weights: w }
}
...
fn make_tau_tuned() -> PLGraph {
    ...
    let i = PLGraph::idx;
    // START: rune-first
    w[i("START")][i("OP")] = 47.0; w[i("START")][i("IDENT")] = 35.0;
    w[i("START")][i("DELIM")] = 18.0;
    // KW
    w[i("KW")][i("BRACE")] = 33.0; w[i("KW")][i("PAREN")] = 21.0;
    w[i("KW")][i("SEMI")] = 20.0; w[i("KW")][i("KW")] = 10.0;
    // IDENT: hub
    w[i("IDENT")][i("PAREN")] = 16.0; w[i("IDENT")][i("ARROW")] = 14.0;
    w[i("IDENT")][i("COLON")] = 14.0; w[i("IDENT")][i("LIT")] = 13.0;
    w[i("IDENT")][i("DOT")] = 10.0; w[i("IDENT")][i("COMMA")] = 10.0;
    w[i("IDENT")][i("OP")] = 10.0; w[i("IDENT")][i("BRACE")] = 5.0;
    w[i("IDENT")][i("SEMI")] = 4.0; w[i("IDENT")][i("END")] = 3.0;
    // COLON: tunnel
    w[i("COLON")][i("TYPE")] = 86.0; w[i("COLON")][i("IDENT")] = 10.0;
    w[i("COLON")][i("COMMA")] = 3.0;
    // SEMI: tunnel
    w[i("SEMI")][i("TYPE")] = 100.0;
    // TYPE
    w[i("TYPE")][i("ARROW")] = 40.0; w[i("TYPE")][i("COMMA")] = 19.0;
    w[i("TYPE")][i("PAREN")] = 16.0; w[i("TYPE")][i("BRACE")] = 10.0;
    w[i("TYPE")][i("OP")] = 5.0;
    // ARROW
    w[i("ARROW")][i("TYPE")] = 24.0; w[i("ARROW")][i("BRACE")] = 24.0;
    w[i("ARROW")][i("KW")] = 16.0; w[i("ARROW")][i("LIT")] = 16.0;
    // DOT: junction
    w[i("DOT")][i("IDENT")] = 50.0; w[i("DOT")][i("DELIM")] = 28.0;
    w[i("DOT")][i("END")] = 16.0; w[i("DOT")][i("KW")] = 6.0;
    // OP
    w[i("OP")][i("LIT")] = 47.0; w[i("OP")][i("PAREN")] = 25.0;
    w[i("OP")][i("KW")] = 9.0; w[i("OP")][i("DELIM")] = 9.0;
    // DELIM: tunnel
    w[i("DELIM")][i("KW")] = 80.0; w[i("DELIM")][i("DELIM")] = 20.0;
    // PAREN
    w[i("PAREN")][i("IDENT")] = 21.0; w[i("PAREN")][i("LIT")] = 14.0;
    w[i("PAREN")][i("ARROW")] = 14.0; w[i("PAREN")][i("COMMA")] = 11.0;
    w[i("PAREN")][i("COLON")] = 11.0; w[i("PAREN")][i("KW")] = 8.0;
    w[i("PAREN")][i("BRACE")] = 8.0; w[i("PAREN")][i("DOT")] = 5.0;
    // BRACE
    w[i("BRACE")][i("KW")] = 38.0; w[i("BRACE")][i("IDENT")] = 28.0;
    w[i("BRACE")][i("DOT")] = 9.0; w[i("BRACE")][i("END")] = 9.0;
    // LIT
    w[i("LIT")][i("COMMA")] = 21.0; w[i("LIT")][i("OP")] = 17.0;
    w[i("LIT")][i("ARROW")] = 13.0; w[i("LIT")][i("PAREN")] = 11.0;
    w[i("LIT")][i("SEMI")] = 6.0;
    // COMMA
    w[i("COMMA")][i("IDENT")] = 42.0; w[i("COMMA")][i("LIT")] = 21.0;
    w[i("COMMA")][i("KW")] = 10.0; w[i("COMMA")][i("OP")] = 10.0;
    w[i("COMMA")][i("TYPE")] = 5.0;
    PLGraph { name: "Futuruna-tuned", weights: w }
}
...
fn main() {
    ...
    let all_langs: Vec<PLGraph> = vec![
        make_lang("Rust"), make_lang("Prolog"), make_lang("Haskell"),
        make_lang("Catala"), make_lang("APL"), make_lang("Forth"),
        make_lang("Idris"), make_lang("Go"), make_lang("Datalog"),
        make_lang("Lean4"),
        make_tau_v3(), make_tau_tuned(),
    ];

    println!("  {:12} {:>8} {:>8} {:>8} {:>5} {:>10} {:>8} {:>8} {:>8}",
        "Language", "S_τ(3)", "JSD", "Φ", "d_eff", "Composite", "λ₁", "λ₂", "λ₃");
    println!("  {:12} {:>8} {:>8} {:>8} {:>5} {:>10} {:>8} {:>8} {:>8}",
        "─".repeat(12), "─".repeat(8), "─".repeat(8), "─".repeat(8),
        "─".repeat(5), "─".repeat(10), "─".repeat(8), "─".repeat(8), "─".repeat(8));
    ...
    for g in &all_langs {
        ...
        println!("  {:12} {:8.4} {:8.4} {:8.3} {:5} {:10.4} {:8.4} {:8.4} {:8.4}",
            g.name, m.mean_s3, m.jsd, m.phi, m.d_eff, comp,
            m.evals[0], m.evals[1], m.evals[2]);

        if m.mean_s3 > best_s.1 { best_s = (g.name, m.mean_s3); }
        if m.jsd > best_j.1 { best_j = (g.name, m.jsd); }
        if m.phi > best_p.1 { best_p = (g.name, m.phi); }
        if m.d_eff > best_d.1 { best_d = (g.name, m.d_eff); }
    ...
    let tau_m = all_metrics.iter().find(|(n, _)| *n == "Futuruna-tuned").unwrap().1.clone();
    println!("  WHO BEATS TAU-TUNED ON ANY AXIS?");
    for (name, m) in &all_metrics {
        if *name == "Futuruna-tuned" || *name == "Futuruna-v3" { continue; }
        let mut beats = Vec::new();
        if m.mean_s3 > tau_m.mean_s3 { beats.push(format!("S_τ ({:.3} vs {:.3})", m.mean_s3, tau_m.mean_s3)); }
        if m.jsd > tau_m.jsd { beats.push(format!("JSD ({:.3} vs {:.3})", m.jsd, tau_m.jsd)); }
        if m.phi > tau_m.phi { beats.push(format!("Φ ({:.3} vs {:.3})", m.phi, tau_m.phi)); }
        if m.d_eff > tau_m.d_eff { beats.push(format!("d_eff ({} vs {})", m.d_eff, tau_m.d_eff)); }
        if !beats.is_empty() {
    ...
    for g in &all_langs {
        ...
        for tok in 0..N_TOK {
            let row_sum: f64 = g.weights[tok].iter().sum();
            if row_sum < 1.0 { continue; }
            if tok == PLGraph::idx("START") || tok == PLGraph::idx("END") { continue; }
    ...
    println!("  {:12} {:>8} {:>6} {:>8} {:>6} {:>8} {:>6} {:>6} {:>6}",
        "Language", "S_τ±", "S_rng", "JSD±", "J_rng", "Φ±", "Φ_rng", "d=2+", "d=3+");
    println!("  {:12} {:>8} {:>6} {:>8} {:>6} {:>8} {:>6} {:>6} {:>6}",
        "─".repeat(12), "─".repeat(8), "─".repeat(6), "─".repeat(8),
        "─".repeat(6), "─".repeat(8), "─".repeat(6), "─".repeat(6), "─".repeat(6));
    ...
    for g_base in [make_tau_v3(), make_tau_tuned()] {
        ...
        for _ in 0..n_trials {
            ...
            j_vals.push(m.jsd);
            p_vals.push(m.phi);
            if m.d_eff >= 2 { d2_count += 1; }
            if m.d_eff >= 3 { d3_count += 1; }
        }
        ...
        let s_range = s_vals.iter().cloned().fold(f64::MAX, f64::min)..=
            *s_vals.iter().max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap();
        let j_range = j_vals.iter().cloned().fold(f64::MAX, f64::min)..=
            *j_vals.iter().max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap();
        let p_range = p_vals.iter().cloned().fold(f64::MAX, f64::min)..=
            *p_vals.iter().max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap();
        println!("  {:12} {:8.3} {:6.3} {:8.3} {:6.3} {:8.3} {:6.3} {:5}% {:5}%",
            g_base.name, s_mean,
    ...
    println!();
    println!("  Ownership/borrowing        MISSING          HIGH        Need for systems programming");
    println!("  Dependent types            MISSING          HIGH        Need for formal verification");
    println!("  Guaranteed termination     MISSING          MEDIUM      | rules can diverge");
    println!("  Algebraic effects          PLACEHOLDER (@)  MEDIUM      @ is declared, not designed");
    println!("  Concurrency                MISSING          HIGH        No actors, channels, async");
    println!("  Metaprogramming            MISSING          MEDIUM      No macros");
    println!("  Array programming          MISSING          LOW         Could be library, not syntax");
    ...
    println!();
    for g in &all_langs {
        let n_active = (0..N_TOK).filter(|&tok| {
            tok != PLGraph::idx("START") && tok != PLGraph::idx("END")
            && g.weights[tok].iter().sum::<f64>() >= 1.0
        }).count();
        let m = evaluate(g);
        println!("  {:12} active_tokens={:2}  S_τ={:.3}  Φ={:.3}",
    ...
    let r_sz_st = if std_sz > 1e-10 && std_st > 1e-10 {
        sizes.iter().zip(staus.iter())
            .map(|(s, t)| (s - mean_sz) * (t - mean_st))
            .sum::<f64>() / (n * std_sz * std_st)
    } else { 0.0 };

    let r_sz_ph = if std_sz > 1e-10 && std_ph > 1e-10 {
        sizes.iter().zip(phis.iter())
            .map(|(s, p)| (s - mean_sz) * (p - mean_ph))
            .sum::<f64>() / (n * std_sz * std_ph)
    } else { 0.0 };
    ...
    if r_sz_st.abs() > 0.7 {
        ...
        println!("  Futuruna's high S_τ may partly reflect having more token types, not better syntax.");
    ...
    println!("    3. Termination is not guaranteed for | rules — Catala has this, Futuruna doesn't");
