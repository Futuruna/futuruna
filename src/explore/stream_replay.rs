...
use super::exact_stream::{
    decode_exact_case_observation_v2, encode_exact_case_observation_v2,
    ExactCaseObservationProposalV2, ExactClosedClassificationV1, ExactEvidenceReducer,
    ExactValidationReceiptDigestV1,
};
use super::run_stream::CanonicalDigest;
use super::transition::TransitionSchemaIdentities;

pub(crate) const EXACT_REPLAY_CLOSURE_BLOB_KIND_V2: &str = "exact-replay-closure-v2";

const REPLAY_MANIFEST_MAGIC_V2: &[u8; 8] = b"FXRPL002";
const MAX_REPLAY_OBSERVATIONS_V2: usize = 65_536;
const MAX_REPLAY_MANIFEST_BYTES_V2: usize = 64 * 1024 * 1024;
const REPLAY_WITNESS_DIGEST_V2: &[u8] = b"futuruna.explore.replay-witness-set.v2";

#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) struct ExactReplayClosureManifestV2 {
    observations: Box<[ExactCaseObservationProposalV2]>,
}

impl ExactReplayClosureManifestV2 {
    pub(crate) fn new(
        ...
        if observations.len() > MAX_REPLAY_OBSERVATIONS_V2 {
            return Err(ExactReplayManifestError::invalid(format!(
                "replay manifest has {} observations; limit is {MAX_REPLAY_OBSERVATIONS_V2}",
                observations.len()
            )));
        }
        ...
        for observation in &observations {
            let bytes = encode_exact_case_observation_v2(observation, transition_schemas)
                .map_err(|error| ExactReplayManifestError::invalid(error.to_string()))?;
            let decoded = decode_exact_case_observation_v2(&bytes, transition_schemas)
                .map_err(|error| ExactReplayManifestError::invalid(error.to_string()))?;
    ...
    pub(crate) fn observations(&self) -> &[ExactCaseObservationProposalV2] {
        &self.observations
    }
}
...
pub(crate) struct ExactReplayClosureValidationV2 {
    normalized_witness_digest: CanonicalDigest,
}

impl ExactReplayClosureValidationV2 {
...
pub(crate) fn validate_exact_replay_closure_v2(
    reducer: &ExactEvidenceReducer,
    manifest: &ExactReplayClosureManifestV2,
    transition_schemas: &TransitionSchemaIdentities,
) -> Result<ExactReplayClosureValidationV2, ExactReplayManifestError> {
    ...
    let expected_ranks = exact_replay_witness_ranks_v2(reducer)?
        .into_vec()
        .into_iter()
        .collect::<BTreeSet<_>>();
    ...
    hasher.update(REPLAY_WITNESS_DIGEST_V2);
    ...
    for observation in manifest.observations.iter() {
        ...
        let bytes = encode_exact_case_observation_v2(&normalized, transition_schemas)
            .map_err(|error| ExactReplayManifestError::invalid(error.to_string()))?;
        hasher.update((bytes.len() as u64).to_le_bytes());
        hasher.update(bytes);
    }
    Ok(ExactReplayClosureValidationV2 {
...
pub(crate) fn exact_replay_witness_ranks_v2(
    reducer: &ExactEvidenceReducer,
    ...
    if ranks.len() > MAX_REPLAY_OBSERVATIONS_V2 {
        return Err(ExactReplayManifestError::invalid(format!(
            "selected replay witness set has {} CaseIds; atomic V2 limit is {MAX_REPLAY_OBSERVATIONS_V2}",
            ranks.len()
        )));
    }
...
pub(crate) fn encode_exact_replay_closure_manifest_v2(
    manifest: &ExactReplayClosureManifestV2,
    transition_schemas: &TransitionSchemaIdentities,
    ...
    let mut encoded_len = REPLAY_MANIFEST_MAGIC_V2
        .len()
        .checked_add(4)
    ...
    for observation in manifest.observations.iter() {
        ...
        encoded_len = encoded_len
            ...
            .ok_or_else(|| ExactReplayManifestError::invalid("replay manifest length overflow"))?;
        if encoded_len > MAX_REPLAY_MANIFEST_BYTES_V2 {
            return Err(ExactReplayManifestError::invalid(format!(
                "replay manifest exceeds {MAX_REPLAY_MANIFEST_BYTES_V2} bytes"
            )));
        }
    ...
    output.extend_from_slice(REPLAY_MANIFEST_MAGIC_V2);
...
pub(crate) fn decode_exact_replay_closure_manifest_v2(
    bytes: &[u8],
    transition_schemas: &TransitionSchemaIdentities,
) -> Result<ExactReplayClosureManifestV2, ExactReplayManifestError> {
    if bytes.len() > MAX_REPLAY_MANIFEST_BYTES_V2 {
        return Err(ExactReplayManifestError::invalid(format!(
            "replay manifest has {} bytes; limit is {MAX_REPLAY_MANIFEST_BYTES_V2}",
            bytes.len()
        )));
    }
    if bytes.len() < REPLAY_MANIFEST_MAGIC_V2.len() + 4
        || &bytes[..REPLAY_MANIFEST_MAGIC_V2.len()] != REPLAY_MANIFEST_MAGIC_V2
    {
        return Err(ExactReplayManifestError::invalid(
            "replay manifest has invalid V2 magic or a truncated header",
        ));
    }
    let mut cursor = REPLAY_MANIFEST_MAGIC_V2.len();
    let count = read_u32(bytes, &mut cursor, "replay observation count")? as usize;
    if count > MAX_REPLAY_OBSERVATIONS_V2 {
        return Err(ExactReplayManifestError::invalid(format!(
            "replay manifest count {count} exceeds {MAX_REPLAY_OBSERVATIONS_V2}"
        )));
    }
    ...
    for _ in 0..count {
        ...
        let observation = decode_exact_case_observation_v2(body, transition_schemas)
            .map_err(|error| ExactReplayManifestError::invalid(error.to_string()))?;
    ...
    let manifest =
        ExactReplayClosureManifestV2::new(observations.into_boxed_slice(), transition_schemas)?;
    let canonical = encode_exact_replay_closure_manifest_v2(&manifest, transition_schemas)?;
