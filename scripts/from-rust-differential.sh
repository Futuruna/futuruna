...
case	covered_supported_subset
numeric_branch_matrix.rs	loops, conditionals, integer arithmetic, deterministic stdout
option_result_pipeline.rs	Option<String>, Result<i64, enum>, parse().map_err(...)? validation
nested_order_totals.rs	nested structs, Vec fields, borrowed helper functions, indexing
btree_rollup_report.rs	BTreeMap<String, i64> aggregation and deterministic reporting
text_transform_matrix.rs	trim, to_lowercase, replace, starts_with, ends_with, format!
enum_loop_rebinding.rs	enum/reference matching plus conditional accumulator rebinding
MANIFEST
}
...
cat "$OUTPUT_FILE" >&2
grep -Fq "From-rust: 6 matched" "$OUTPUT_FILE" \
    || fail "differential lane did not report expected summary: From-rust: 6 matched"
