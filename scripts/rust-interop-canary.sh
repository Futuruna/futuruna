...
assert_lib_header() {
    ...
    grep -Fq "// Generated by runa --lib" "$rust_source" \
        || fail "Generated library source did not identify runa --lib output"
}

assert_dependency_guidance() {
    local rust_source="$1"
    local stderr_file="$2"
    local expected_dep="$3"

    assert_lib_header "$rust_source"
    grep -Fq "Required Cargo dependencies for consumers:" "$rust_source" \
        || fail "Generated library source did not include required Cargo dependency guidance"
    grep -Fq "$expected_dep" "$rust_source" \
        || fail "Generated library source did not include expected dependency: $expected_dep"
    grep -Fq "Cargo dependencies required by generated Rust:" "$stderr_file" \
        || fail "runa lib stderr did not include required Cargo dependency guidance"
    grep -Fq "$expected_dep" "$stderr_file" \
        || fail "runa lib stderr did not include expected dependency: $expected_dep"
    grep -Fq "consuming Cargo.toml [dependencies]" "$stderr_file" \
        || fail "runa lib stderr did not tell users where to add dependencies"
}
...
run_basic_consumer_canary() {
    ...
    assert_lib_header "$work_dir/futuruna_lib.rs"
...
run_external_crate_consumer_canary() {
    ...
    "$RELEASE_RUNA" lib "$fixture" > "$work_dir/src/futuruna_lib.rs" 2> "$work_dir/runa-lib.stderr"
    cat "$work_dir/runa-lib.stderr" >&2
    assert_dependency_guidance "$work_dir/src/futuruna_lib.rs" "$work_dir/runa-lib.stderr" 'regex = "1"'
...
run_package_crate_consumer_canary() {
    ...
    "$RELEASE_RUNA" lib "$fixture" > "$lib_dir/src/lib.rs" 2> "$work_dir/runa-lib.stderr"
    cat "$work_dir/runa-lib.stderr" >&2
    assert_dependency_guidance "$lib_dir/src/lib.rs" "$work_dir/runa-lib.stderr" 'regex = "1"'
...
run_missing_dependency_guidance_canary() {
    local fixture="$1"
    local work_dir="$TMP_DIR/missing-dependency"
    mkdir -p "$work_dir/src"
    ...
    cat > "$work_dir/Cargo.toml" <<'TOML'
[package]
name = "futuruna-rust-interop-missing-dependency"
version = "0.1.0"
edition = "2021"
...
RS
    ...
    echo
    echo "[rust-interop] expecting missing dependency failure from Cargo consumer"
    local status=0
    (
        cd "$work_dir"
        CARGO_NET_OFFLINE=true "$CARGO_BIN" check --quiet
    ) > "$work_dir/cargo.out" 2> "$work_dir/cargo.err" || status=$?
    if [[ "$status" -eq 0 ]]; then
        fail "Rust consumer unexpectedly compiled without required regex dependency"
    fi
    grep -Fq "regex" "$work_dir/cargo.err" \
        || fail "Missing-dependency Cargo failure did not mention regex"

    echo
    echo "[rust-interop] Missing-dependency guidance canary passed for: $fixture"
}
...
run_missing_dependency_guidance_canary "$EXTERNAL_FIXTURE"
