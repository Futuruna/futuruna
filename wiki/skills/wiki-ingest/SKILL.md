...
Before ingesting any file, check `wiki/.raw/.manifest.json` to avoid re-processing unchanged sources.

```bash
# Check if manifest exists
[ -f wiki/.raw/.manifest.json ] && echo "exists" || echo "no manifest yet"
```
...
{
  "sources": {
    "wiki/.raw/articles/article-slug-2026-07-18.md": {
      "hash": "abc123",
...
4. **Save** to `wiki/.raw/articles/[slug]-[YYYY-MM-DD].md` with a frontmatter header:
   ```markdown
   ---
   source_url: [url]
   fetched: [YYYY-MM-DD]
   ---
   ```
5. Proceed with **Single Source Ingest** starting at step 2 (file is now in `wiki/.raw/`).
...
3. **Save** the description to `wiki/.raw/images/[slug]-[YYYY-MM-DD].md`:
   ...
   ```
4. Copy the image to `wiki/_attachments/images/[slug].[ext]` if it's not already in the vault.
...
- Do not modify anything in `wiki/.raw/`. These are immutable source documents.
