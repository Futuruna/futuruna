...
**Resolve the image:**
- If URL (starts with `http`): download with `curl -sL [url] -o wiki/_attachments/images/canvas/[filename]`
  Derive filename from URL path, or use `img-[timestamp].jpg` if unclear.
- If local path outside vault: `cp [path] wiki/_attachments/images/canvas/`
- If already vault-relative: use as-is.

Create `wiki/_attachments/images/canvas/` if it doesn't exist.
...
- Copy to `wiki/_attachments/pdfs/canvas/` if outside vault.
...
2. If not found or empty: use `find` with correct precedence (parentheses required. Without them `-newer` only binds to the last `-name` clause):
   ...
   find wiki/_attachments/images -newer /tmp/ten-min-ago \( -name "*.png" -o -name "*.jpg" \)
   ```
...
If `wiki/canvases/.recent-images.txt` exists, append any new image path written to `wiki/_attachments/images/` during this session (one path per line, keep last 20).
...
3. Create `wiki/_attachments/images/canvas/` for downloaded/copied images.
