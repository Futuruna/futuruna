...
For this existing repository, follow `docs/repository-layout.md` and
`wiki/getting-started.md`. Preserve root `AGENTS.md` and `CLAUDE.md`; the
new-vault bootstrap templates below do not replace the repository agent policy.
...
```
vault/
├── wiki/.raw/       # Layer 1: immutable source documents
├── wiki/            # Layer 2: knowledge notes and supporting material
└── wiki/workflows.md # Layer 3: workflow reference
```
...
Dot-prefixed folders (`wiki/.raw/`) are hidden in Obsidian's file explorer and graph view. Use this for source documents.
...
6. Create `wiki/_templates/` files for each note type.
...
- Wikilinks use [[Note Name]] format: use explicit paths when names overlap with repository documentation
...
- Archive: move cold sources to .archive/ to keep wiki/.raw/ clean
```
...
7. Never modify wiki/.raw/ sources
