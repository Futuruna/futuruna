...
| Plugin | Purpose |
|--------|---------|
| **Templater** | Auto-populate frontmatter on note creation from `wiki/_templates/`. |
...
The Obsidian Web Clipper browser extension converts web articles to markdown and sends them to `wiki/.raw/` in one click.

Install for Chrome, Firefox, or Safari from the Obsidian website.

Set the default folder to `wiki/.raw/` in the extension settings.
