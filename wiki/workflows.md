# Wiki workflows — LLM Wiki Schema
...
> Repository paths in this guide are relative to the Futuruna checkout root.
> Start with [[wiki/index|the wiki index]]; maintained product docs live under `docs/`.
> Futuruna is already bootstrapped. New-vault setup templates below are reference
> material; preserve the existing root `AGENTS.md` and `CLAUDE.md`.
...
Also install the **Obsidian Web Clipper** browser extension. It converts web articles to markdown and sends them to `wiki/.raw/` in one click. Available for Chrome, Firefox, and Safari.
...
```
futuruna/                   # Open this directory as the vault
├── docs/                   # Maintained source contracts
├── wiki/
│   ├── .raw/               # Immutable source imports
│   ├── _templates/         # Note templates
│   ├── _attachments/       # Note images and PDFs
│   ├── skills/             # Repository workflow copies
│   ├── workflows.md        # This reference
│   ├── index.md            # Catalog of knowledge notes
│   ├── log.md              # Chronological maintenance record
│   ├── hot.md              # Recent context
│   ├── sources/            # Summaries with source provenance
│   ├── entities/
│   ├── concepts/
│   ├── comparisons/
│   ├── questions/
│   └── meta/               # Dashboards and conventions
└── .obsidian/              # Shared vault configuration
```

### Rules

- `wiki/.raw/` is read-only. Never modify source files.
...
type: meta
title: "Hot Cache"
updated: 2026-07-18T07:30:00+02:00
---

# Recent Context

## Last Updated
2026-07-18 — Ingested 3 new YouTube transcripts
...
related:
  - "[[Other Page]]"
sources:
  - "[[wiki/.raw/articles/source-file.md]]"
---
```
...
6. Create `wiki/_templates/` with templates for each note type.
...
```
vault/
├── wiki/.raw/              # crawl exports, analytics, GSC data
├── wiki/
...
```
vault/
├── wiki/.raw/              # README, git log exports, code dumps
├── wiki/
...
```
vault/
├── wiki/.raw/              # meeting transcripts, Slack exports, docs
├── wiki/
...
```
vault/
├── wiki/.raw/              # journal entries, articles, voice transcripts
├── wiki/
...
```
vault/
├── wiki/.raw/              # PDFs, web clips, raw notes
├── wiki/
...
```
vault/
├── wiki/.raw/              # chapter notes, highlights, exercises
├── wiki/
...
- Wikilinks use [[Note Name]] format — use explicit paths when note names overlap with repository documentation
- wiki/.raw/ contains source documents — never modify them
- wiki/index.md is the master catalog — update on every ingest
- wiki/log.md is append-only — new entries go at the TOP, never edit past entries

## Operations

- Ingest: drop source in wiki/.raw/, say "ingest [filename]"
...
Trigger: user drops a file into `wiki/.raw/` or pastes content.
...
10. Append to `wiki/log.md` (new entries at the TOP):
    ```markdown
    ## [2026-07-18] ingest | Source Title
    - Source: `wiki/.raw/articles/filename.md`
...
Note: these differ from the wiki graph CSS color scheme. See `wiki/skills/canvas/references/canvas-spec.md` for the full canvas color table.
...
8. Never modify wiki/.raw/ sources
