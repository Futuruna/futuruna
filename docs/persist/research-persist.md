...
~ big_orders = order_feed |> filter(|o| o.total > 5000)
~ big_orders | o -> {
    @ print("Large order: " + show(o))
}
```

`watch(Type)` returns a stream that emits whenever a fact is asserted or retracted.
...
```runa
# FactEvent(kind: EventKind, fact: a)
# EventKind = Asserted | Retracted
```
...
~ changes | event -> {
    match event.kind {
        | Asserted -> @ print("New user: " + show(event.fact))
        | Retracted -> @ print("Removed: " + show(event.fact))
    }
}
```

Or for convenience, `watch` could emit the facts directly (assert-only, most common case):
```runa
~ new_users = watch(User)            -- emits on assert only
~ all_changes = watch_all(User)      -- emits FactEvent with kind
```
