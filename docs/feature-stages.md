...
| Bounded `? explore` declarations, typed query analysis, and exact-finite execution | Experimental | The compiler parses and type-checks this analysis declaration. `runa explore` executes the supported finite subset with explicit closure status. Its durable slice accepts an owner-supplied `--run-state`, treats checked source probing and candidate evaluation as phase zero, journals exact frontier deltas, and supports bounded pause/resume from the authoritative journal cursor. A separately admitted publication phase may emit a bounded cursor-bearing snapshot with its checked configuration. Durable runs may explicitly opt into total current-evidence case-DAG disclosure with `--case-graph full`; that request, its retention authorization, and the fixed publication schema are immutable run identity. A query may declare `observe mechanisms with CALLABLE`; the compiler seals one pure, total `(State, Context) -> Observation` endpoint template into checked IR, independently of output fields. Isolated Before/After observer replay and mechanism publication are not implemented yet. Detached followers, parallel workers, chunked terminal publication, and the full public `futuruna.explore.v1` result schema remain later slices. |
...
| `runa explore` | Experimental | Executes a selected bounded query through the exact-finite reference backend. The bounded one-shot `--case-limit` form and the resumable stream are two presentation/lifecycle adapters over that same checked transition IR and exact evaluator; the one-shot JSON result remains unavailable. On macOS, the observable path uses an explicit private `--run-state` with `--time-limit` and/or `--pause-after probes`, validates immutable run identity on resume, and emits a typed `futuruna.explore.invocation.v1` receipt. The append-only paused journal cursor is the authoritative resume checkpoint. Snapshot v6 and its optional case DAG are a separately admitted materialized view with a fixed 256 MiB accounted working-set envelope under the normal 80% CPU/RAM ceiling; deadline or admission refusal instead returns typed `JournalOnlyCheckpoint` / JSON artifact kind `journal_checkpoint` with `snapshot.status = deferred` and no canonical snapshot payload or blob. If an admitted publisher reports capacity, it emits the separate bounded `snapshot_unavailable` receipt for that cursor; this is not a partial snapshot or a permanence claim. The next resumed invocation services pending snapshot-publication debt before more semantic work and stops at `snapshot_catch_up`, preventing repeated time slices from indefinitely outrunning publication. An admitted `--case-graph full` snapshot still includes the complete total current-evidence DAG or typed `capacity_limited` status, never a graph prefix. Explicit `--finalize` with a time limit can atomically replay, publish `futuruna.explore.exact-answer.v5`, and seal only answers that fit its current bounded envelope; when a graph was requested, sealing additionally requires that graph to be included and closed. Mechanism observation is selected by the query-owned `observe mechanisms with CALLABLE` clause, never by positional `output.show` indexes. The execution path carries the checked endpoint template but does not yet replay it; mechanism evidence remains unavailable, and mechanism checkpoints, a public mechanism DAG, and mechanism-aware terminal publication remain future slices. Snapshot/checkpoint deferral and unavailable receipts are operational and do not change evidence, graph capacity, or run identity. CPU and RAM are budgeted independently beneath an 80% operational ceiling that preserves at least 20% of CPU and `max(20% of RAM, 1 GiB)` for the host, with sampled early-trip circuit breakers plus a process-start Rust allocation-request cap rather than a hard instantaneous quota. Durable supervision on other platforms, detached following, multi-worker execution, general mechanism DAG publication, and resumable/chunked finalization remain in progress; ordinary exact snapshots still report mechanism evidence as `unavailable_deferred`. |
