...
| Bounded `? explore` declarations, typed query analysis, and exact-finite execution | Experimental | The compiler parses and type-checks this analysis declaration. `runa explore` executes the supported finite subset with explicit closure status. Its first durable slice accepts an owner-supplied `--run-state`, treats checked source probing and candidate evaluation as phase zero, journals exact frontier deltas, supports bounded pause/resume, and emits a bounded cursor-bearing answer preview with its checked configuration. Detached followers, parallel workers, mechanism replay, chunked terminal publication, and the full public `futuruna.explore.v1` result schema remain later slices. |
...
| `runa explore` | Experimental | Executes a selected bounded query through the exact-finite reference backend. The human-only compatibility path uses `--case-limit`; its JSON result remains unavailable. On macOS, the observable path uses an explicit private `--run-state` with `--time-limit` and/or `--pause-after probes`, validates immutable run identity on resume, and emits a typed `futuruna.explore.invocation.v1` receipt containing the exact published `futuruna.explore.snapshot.v4` checkpoint. Snapshot publication and pause are separate ordered journal records with inspectable cursors. Rows and configuration values are identity-bound bounded previews; counts retain exact/lower-bound labels independently of display truncation. Explicit `--finalize` with a time limit can atomically replay, publish `futuruna.explore.exact-answer.v3`, and seal only answers that fit its current bounded envelope; larger answers publish a checkpoint and pause with typed `FinalizationLimit` details. CPU and RAM are budgeted independently beneath an 80% operational ceiling, with sampled early-trip circuit breakers plus a process-start Rust allocation-request cap rather than a hard instantaneous quota. Durable supervision on other platforms, detached following, multi-worker execution, case/mechanism DAG publication, and resumable/chunked finalization remain in progress. |
