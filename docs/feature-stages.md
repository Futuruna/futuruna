...
| `runa meta` | Preview | Resolves typed role-to-binding references from machine-readable `--@label`, `--@meta`, and legacy `--@source` comments; indexes raw-text/code spans and symbols; and supports type/role sweeps plus versioned `--json` output without changing language semantics. Output shape may still evolve as audit and explanation tooling grows. |
