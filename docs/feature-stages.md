...
| `runa explore` | Experimental | Selects one named relational query, opens or resumes its authenticated journal under an explicit `--run-state`, executes one time/resource-bounded semantic slice, and incrementally publishes authorized views and case/mechanism artifacts to a separate `--output` tree. Identical completed resumes are no-ops. The selected- and same-question chosen-view-target paths, endpoint mechanism replay, post-mechanism views, structural mechanism/profile catalog, and factorized case/starter-support summaries are implemented; large definition lanes have independent bounded cursors. Publication v8 auto-schedules a separately authenticated `mechanisms/<request>.starters.ndjson` lane for whole structural mechanisms when a compatible checked lossless selected-input, each-case view already authorizes `case_id`, `context`, `before` and `after`. Its canonical k-way pager starts with at most 64 typed members, adaptively shortens a page to fit publication `max_line_bytes`, fails explicitly if one member alone is too large, and uses memory proportional to contributing signatures plus that page. Each mechanism closure independently certifies exact case and distinct-starter counts. A running invocation advances and publishes bounded internal micro-slices, so durable results can emerge before the outer invocation returns; a separate observer/tail command is not yet exposed. Admission-scoped targets, authored starter-export syntax, typed node/edge starter projections and fixed-fan-in external merging remain deferred. There is no public probe phase, ordinal case-space contract, or compatibility promise for v0 artifacts. Invocation CPU/RAM limits are operational policy and currently preserve at least 20 percent of CPU and RAM for the host; the containment supervisor is presently macOS-only. Parallel workers, symbolic closure and the broad Personskat audit remain in progress. |
