...
| `runa meta` | Preview | Scans machine-readable meta comments such as `--@source`, `--@begin`, and `--@end` to report source/text/code spans without changing language semantics. Output shape may still evolve as audit and explanation tooling grows. |
