...
| Bounded `? explore` declarations and typed query analysis | Experimental | The compiler parses and type-checks this analysis declaration. Ordinary execution and code generation treat it as a declaration; search execution, replay, result reporting, and a dedicated CLI command arrive in later implementation slices. |
