...
| Bounded `? explore` declarations, typed query analysis, and exact-finite execution | Experimental | The compiler parses and type-checks this analysis declaration. `runa explore` executes the supported finite subset with explicit closure status. Its durable slice accepts an owner-supplied `--run-state`, treats checked source probing and candidate evaluation as phase zero, and atomically journals each closed `CaseId` classification together with its canonical Context/Before/After transition when that case is constructible. It supports bounded pause/resume from the authoritative journal cursor. A separately admitted publication phase may emit bounded cursor-bearing snapshot v7. Durable runs independently request the total current-evidence search decision DAG with `--search-decision-dag full` and the canonical semantic transition graph with `--semantic-transition-graph full`; both choices, retention authorization, and fixed publication schemas are immutable run identity. Counts `U_D`, `U_C`, `U_T`, `D_C`, `D_T`, `M_C`, and `M_T` remain public as exact values or honest lower bounds even when either graph is omitted or reported as typed `capacity_limited` status. A query may declare `observe mechanisms with CALLABLE`; the compiler seals one pure, total `(State, Context) -> Observation` endpoint template into checked IR, independently of output fields. Mechanism replay and mechanism-DAG publication are not implemented yet. Detached followers, parallel workers, chunked terminal publication, and the full public `futuruna.explore.v1` result schema remain later slices. |
...
| `runa explore` | Experimental | Executes a selected bounded query through the exact-finite reference backend. The bounded one-shot `--case-limit` form and the resumable stream are two presentation/lifecycle adapters over the same checked transition IR and exact evaluator; the one-shot JSON result remains unavailable. On macOS, the observable path uses an explicit private `--run-state` with `--time-limit` and/or `--pause-after probes`, validates immutable run identity on resume, and emits a typed `futuruna.explore.invocation.v1` receipt. The append-only paused journal cursor is the authoritative resume checkpoint. Snapshot v7 is a separately admitted materialized view with a fixed 256 MiB accounted working-set envelope under the normal 80% CPU/RAM ceiling; deadline or admission refusal instead returns typed `JournalOnlyCheckpoint` / JSON artifact kind `journal_checkpoint` with `snapshot.status = deferred` and no canonical snapshot payload or blob. If the admitted outer snapshot publisher reports operational capacity, it emits the separate bounded `snapshot_unavailable` receipt for that cursor. The next resumed invocation services pending snapshot-publication debt before more semantic work and stops at `snapshot_catch_up`. Independently requested search-decision and semantic-transition graphs publish either their complete current canonical object or typed `capacity_limited` status with no prefix. That graph status satisfies the publication request and does not prevent explicit `--finalize` from atomically replaying, publishing `futuruna.explore.exact-answer.v6`, and sealing an otherwise complete answer. Mechanism observation is selected by the query-owned `observe mechanisms with CALLABLE` clause, never by positional `output.show` indexes. The checked endpoint template is not yet replayed; mechanism evidence remains unavailable, and mechanism checkpoints, a public mechanism DAG, and mechanism-aware terminal publication remain future slices. Snapshot/checkpoint deferral and unavailable receipts are operational and do not change evidence, graph capacity, or run identity. CPU and RAM are budgeted independently beneath an 80% operational ceiling that preserves at least 20% of CPU and `max(20% of RAM, 1 GiB)` for the host, with sampled early-trip circuit breakers plus a process-start Rust allocation-request cap rather than a hard instantaneous quota. Durable supervision on other platforms, detached following, multi-worker execution, general mechanism DAG publication, and resumable/chunked finalization remain in progress; ordinary exact snapshots still report mechanism evidence as `unavailable_deferred`. |
