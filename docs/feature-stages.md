...
| `runa meta` | Preview | Resolves typed role-to-binding references from machine-readable `--@label`, `--@meta`, and legacy `--@source` comments; exposes ground metadata values as structural constructor/collection/primitive trees with recursively typed descendant paths and typed aggregate attachments; indexes raw-text/code spans and symbols; and supports type/role sweeps over files or recursive source trees with versioned `--json` output, without changing language semantics. Output shape may still evolve as audit and explanation tooling grows. |
