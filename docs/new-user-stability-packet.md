...
| Translation-checked slice | A compiler output is checked against an independently derived source obligation for a narrow shape. |
...
| Importable local libraries | Production-ready for flat/qualified local import contract | Downstream canary, import expectations, first-run import smoke, lint-library gates | Import normalization/export filtering is mostly trusted code with snapshots and diagnostics | Next high-value formal slice: translation-check import graph normalization and exported set preservation. |
...
| 2 | Import normalization preserves exported declarations and excludes script-only/import-time behavior | Downstream canaries, import expectations, and the first-hour private/missing import diagnostics defend behavior | Add a translation-checking slice for source import graph -> normalized module/export graph. |
