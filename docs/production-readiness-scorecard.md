...
| `runa from-rust` tooling | Preview | Work through the Production Promotion Checklist in `docs/from-rust-contract.md`: keep the 35-fixture FRSS-v0 example corpus, 9-fixture mint-blocking downstream canary, and 6-case generated supported-subset differential lane matching Rust output exactly; freeze FRSS-v0 or a successor as a stable contract; keep broadening external/downstream and differential evidence; stabilize `from-rust --verify` output; and keep arbitrary Rust crate translation outside the claim unless separately promoted. | L | 4 |
