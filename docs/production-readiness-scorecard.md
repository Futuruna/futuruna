...
| `runa from-rust` and `from-rust --verify` | Research-grade | Feature stage is experimental; CI runs the example transpiler validation with 32 exact supported matches and 0 explicit expected-unsupported adversarial fixtures. The supported set now includes the checked-in nested pattern fixture (recursive `Box<T>` enum constructors plus ordered two-reference tuple matches lowered to nested Futuruna matches), the generic trait fixture (`Functor` associated-type calls over `Option`/`Result`, generic higher-order functions, `impl Fn` composition, and generic `Pair` methods), the recursive ownership fixture (`Vec<Box<T>>`, inherent impl methods, functional `&mut self` field-push lowering, and narrow recursive `Option<&T>` search translated as value `Option(T)`), and the stateful iterator/map grouping subset for tuple-key `sort_by`, Fibonacci-style `scan(...).collect()`, and `entry(...).or_insert_with(Vec::new).push(...)`. Unsupported shapes outside these checked subsets still fail closed with stable diagnostic categories documented in `docs/from-rust-contract.md`. | Keep the zero-XFAIL corpus exact; add broader consumer-shaped fixtures and define the compatibility boundary before promotion. |
...
| `runa from-rust` tooling | Research-grade | Keep the supported fixture subset matching Rust output exactly; add broader consumer-shaped fixtures beyond the adversarial corpus; keep stable unsupported diagnostics and CI blocking. | L | 3 |
