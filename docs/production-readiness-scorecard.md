...
| `runa from-rust` and `from-rust --verify` | Research-grade | Feature stage is experimental; CI runs the example transpiler validation with 31 exact supported matches and 1 explicit expected-unsupported adversarial fixture. The supported set now includes the checked-in generic trait fixture (`Functor` associated-type calls over `Option`/`Result`, generic higher-order functions, `impl Fn` composition, and generic `Pair` methods), the recursive ownership fixture (`Vec<Box<T>>`, inherent impl methods, functional `&mut self` field-push lowering, and narrow recursive `Option<&T>` search translated as value `Option(T)`), and the stateful iterator/map grouping subset for tuple-key `sort_by`, Fibonacci-style `scan(...).collect()`, and `entry(...).or_insert_with(Vec::new).push(...)`. Remaining adversarial unsupported shapes fail closed with stable diagnostic categories documented in `docs/from-rust-contract.md`. | Shrink the expected-unsupported fixture set by broadening exact-match support, or preserve unsupported categories as an explicit compatibility boundary before promotion. |
...
| `runa from-rust` tooling | Research-grade | Keep the supported fixture subset matching Rust output exactly; reduce expected-unsupported adversarial fixtures by implementing support for one category at a time; keep stable unsupported diagnostics and CI blocking. | L | 3 |
