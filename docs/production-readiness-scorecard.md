...
| `runa from-rust` and `from-rust --verify` | Research-grade | Feature stage is experimental; CI runs the example transpiler validation with 27 exact supported matches and 5 explicit expected-unsupported adversarial fixtures. Current adversarial unsupported shapes fail closed with stable diagnostic categories documented in `docs/from-rust-contract.md`. | Shrink the expected-unsupported fixture set by broadening exact-match support, or preserve unsupported categories as an explicit compatibility boundary before promotion. |
