...
| `@ persist`, SQL-backed facts, watches, migrations, and transactional storage | Research-grade | Storage canaries exist and M26b is active; typed columns, SQL-backed `findall`, and persisted retract codegen coverage exist, but phase-B children remain open. | Finish M26b phase B (`td-c0a7a1`, `td-c4282c`, `td-13a997`, `td-25667e`) and add storage canary/roundtrip policy. |
