...
| Rust interop and Rust-facing integration | Preview | Feature stage is preview; CI runs `from-rust --test examples/from-rust/`; `runa lib` export shape has an artifact expectation for public/private item boundaries; the Rust interop canary compiles generated `runa lib` output into a plain Rust consumer and calls exported structs, enums, borrowed params, lists, `Option`, and `Result`; the same lane compiles an offline Cargo consumer for external-crate generated code through `@ depend`, `@ use`, raw Rust helpers, and regex-backed stdlib codegen. | Freeze package-layout/manifest expectations and richer FFI boundaries before promotion. |
...
| Rust interop and integration | Preview | Keep the Rust consumer and offline external-crate canaries blocking; freeze package-layout expectations and supported manifest/FFI patterns. | L | 4 |
