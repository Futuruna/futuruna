...
| `runa from-rust` tooling | Preview | Work through the Production Promotion Checklist in `docs/from-rust-contract.md`: keep the 35-fixture supported subset and mint-blocking downstream canary matching Rust output exactly, version the supported subset as a stable contract, add broader external/downstream consumer evidence, add subset-focused differential or proof-backed checking, and keep arbitrary Rust crate translation outside the claim unless separately promoted. | L | 4 |
