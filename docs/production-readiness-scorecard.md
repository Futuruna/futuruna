...
| `runa from-rust` and `from-rust --verify` | Research-grade | Feature stage is experimental; CI runs the example transpiler validation with 30 exact supported matches and 2 explicit expected-unsupported adversarial fixtures. The supported set now includes the checked-in recursive ownership fixture (`Vec<Box<T>>`, inherent impl methods, functional `&mut self` field-push lowering, and narrow recursive `Option<&T>` search translated as value `Option(T)`) plus the stateful iterator/map grouping subset for tuple-key `sort_by`, Fibonacci-style `scan(...).collect()`, and `entry(...).or_insert_with(Vec::new).push(...)`. Remaining adversarial unsupported shapes fail closed with stable diagnostic categories documented in `docs/from-rust-contract.md`. | Shrink the expected-unsupported fixture set by broadening exact-match support, or preserve unsupported categories as an explicit compatibility boundary before promotion. |
