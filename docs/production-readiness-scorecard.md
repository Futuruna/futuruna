...
| `runa from-rust` tooling | Preview | Keep the 35-fixture supported subset and mint-blocking downstream canary matching Rust output exactly; require every newly promoted Rust shape to add supported or expected-unsupported downstream coverage; keep arbitrary Rust crate translation outside the claim. | M | 3 |
