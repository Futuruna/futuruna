...
| `runa from-rust` and `from-rust --verify` | Production-ready | Stable feature stage for FRSS-v0, the named From-Rust Single-File Supported Subset contract in `docs/from-rust-contract.md`; it is not arbitrary Rust crate translation. CI runs the example transpiler validation with 35 exact supported matches and 0 explicit expected-unsupported adversarial fixtures. Mint also runs a clean-directory downstream canary with 9 consumer-shaped exact matches and 17 expected-unsupported fail-closed diagnostics for ownership, generics, error remapping, iterator state machines, tuple reference patterns, effectful `std` APIs, async/threading, unsafe, external-crate, unsupported-module, unsupported-macro, unsupported-format-spec, unsupported-rust-item, and unsupported-rust-expr boundaries, plus a generated seed-stable supported-subset differential lane with 24 exact matches by default, checked-in source-shape-family mapping, coverage metrics, replay artifacts, and minimization notes. `docs/from-rust-evidence-manifest.md` maps every current supported source-shape claim to exact-match fixture or generated-lane evidence. `from-rust --verify` has stable summary lines for supported matches, recognized unsupported categories, Rust parse/compile/run failures, translated Futuruna parse failures, and output divergence, with CLI coverage for supported success, Rust parse failure, Rust compile failure, `unsafe-rust`, `async-threading`, `unsupported-effect`, `unsupported-module`, `unsupported-macro`, `unsupported-format-spec`, `unsupported-rust-expr`, harness-level `translated-parse-failed`, harness-level `mismatch`, and help text. The supported set includes three example-corpus consumer-shaped single-file workflows for config parsing/validation, invoice totals, and event rollups, plus downstream production-corpus growth for nested customer/order data, error-row pipelines, deterministic inventory reporting, text normalization, and enum/reference loop aggregation with conditional accumulator rebinding. It also includes the checked-in nested pattern fixture (recursive `Box<T>` enum constructors and ordered two-reference tuple matches lowered to nested Futuruna matches), generic trait fixture (`Functor` associated-type calls over `Option`/`Result`, generic higher-order functions, `impl Fn` composition, and generic `Pair` methods), recursive ownership fixture (`Vec<Box<T>>`, inherent impl methods, functional `&mut self` field-push lowering, and narrow recursive `Option<&T>` search translated as value `Option(T)`), and stateful iterator/map grouping subset for tuple-key `sort_by`, Fibonacci-style `scan(...).collect()`, and `entry(...).or_insert_with(Vec::new).push(...)`. Unsupported shapes outside FRSS-v0 still fail closed with stable diagnostic categories documented in `docs/from-rust-contract.md`. | Keep the stable FRSS-v0 contract green; add evidence-manifest rows, exact-match fixtures or generated-lane cases, and compatibility-guide notes before expanding supported shapes; keep arbitrary Rust crate translation outside the claim unless separately promoted. |
...
| `runa from-rust` tooling | Production-ready | Keep the 35-fixture FRSS-v0 example corpus, 9-fixture mint-blocking downstream canary, 24-case default seed-stable supported-subset differential lane, source-shape evidence manifest, and stable `from-rust --verify` summary lines matching their contracts; broaden external/downstream, differential, and verify failure-category evidence only with fixture-first compatibility updates; keep arbitrary Rust crate translation outside the claim unless separately promoted. | M | 4 |
...
[from-rust-contract.md](from-rust-contract.md) promotes `runa from-rust` and
`from-rust --verify` to production-ready for the documented FRSS-v0 single-file
validation boundary. Arbitrary Rust crate translation remains outside the
production claim unless it gets a separate documented contract, canaries, and
promotion packet.
...
> importable local-library consumer shape, FRSS-v0 single-file
> Rust-to-Futuruna translation/verification, and differential/generative
> compiler hardening lane are production-ready for small to medium programs that stay
> inside documented syntax, stdlib, named-scope stream lifetime, actor,
> effect-handler, local import, export, import-hygiene, and FRSS-v0 source
> contracts, with
...
> Futuruna as a whole is production-ready across storage, proof-backed compiler
> checking, WASM build artifacts, and arbitrary Rust crate translation.
