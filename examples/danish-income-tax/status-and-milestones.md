...
Current test-selection slice: `td-9b7413` (`runa test --kind scenario --jobs 6 examples/danish-income-tax` kører de 178 scenariefiler parallelt uden de 38 særskilte auditfiler; `--kind audit` bevarer auditpakken som en eksplicit milepæls- eller publiceringsgate, mens udeladt `--kind` fortsat kører alle `.runa`-filer som hidtil)
