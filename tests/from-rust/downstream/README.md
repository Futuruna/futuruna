...
effectful `std` APIs, async/threading, unsafe blocks, external crate imports,
...
outside the checked macro subset, unchecked assertion macros, Rust module
declarations outside the flat single-file boundary, plus top-level Rust item
and expression fallback shapes with no checked lowering.
